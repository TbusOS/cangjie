...
装了 [llm-wiki](https://github.com/TbusOS/llm-wiki) 才用。**不装不影响 Whetstone。**

llm-wiki 是**人看的**知识库(交叉链接 markdown + Web UI)。Whetstone 产出的是**给 agent 用的** skill 包。
同一条知识可以两头都有:wiki 页给人读来龙去脉,skill 包给 agent 执行。

## 映射(Whetstone → llm-wiki 页)

| Whetstone | llm-wiki 页 |
|---|---|
